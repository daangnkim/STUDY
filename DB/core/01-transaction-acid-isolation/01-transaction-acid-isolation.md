...
데이터베이스 관리 시스템은 데이터베이스가 **항상 정확하고 일관된 상태를 유지**할 수 있도록 다양한 기능을 제공. 이중 하나가 트랜잭션.

트랜잭션은 **하나의 논리적인 작업 단위**를 구성하는 연산들의 집합이다. 데이터베이스에서 **논리적인 작업의 단위**이자 장애가 발생했을 때 **데이터를 복구하는 작업의 단위**이기도 하다.
...
활성화 : 트랜잭션 수행중인 상태
부분완료 : 최종 결과를 데이터베이스에 반영하지 않은 상태
완료 : 최정 결과를 데이터베이스에 반영한 상태
실패 : 하드웨어, 소프트웨어의 문제, 트랜잭션 오류 등으로 트랜잭션 수행이 중단된 상태
철회 : rollback 연산 실행. 트랜잭션이 수행되기 전의 데이터베이스 상태로 되돌아감. 하드웨어나 소프트웨어 오류로 철회된 경우 트랜잭션 다시 시작. 트랜잭션 오류인 경우 철회된 트랜잭션 폐기.
